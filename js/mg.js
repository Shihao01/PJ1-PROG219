...
function MG(ob, w, h) {
	...
	this.w = w || 20;
	this.h = h || 20;
	this.grid_size = 20;		// wedith of maze
	this.grid_str = "";		// maze code type not using now 
	this.grids = [];		// statis of every box，about[0, 15]
	this.grid_ob = [];		// DOM of box
	this.is_moved = false;		// does player moved
	this.mark_history = false;	// does they mark red for passed way
	this.mark_history2 = false;	// short cut not useing now
	this.chkCanvasTagSupport();	// check canvas tag support
}
...
MG.prototype = {
	...
	set: function (sets) {
		// setting width and height of map
	...
	create: function () {
		...
		if (typeof console != "undefined") {
			console.log("timeuse: " + (t1 - t0) + "ms");
		}
		return this;
	},
	_walk: function (startPos) {
		// ruing map from startpoint
		this._walk_history = [];
		this._walk_history2 = [];
		var current_pos = startPos;
		while (this._getNext0() != -1) {
			// if any box haven't been
	...
	_getTargetSteps: function (current_pos) {
		// get side of box haven't been and can be go at next step.
		// can be there of not if not -1，
		// return as a number set.
		var p = 0,
			a = [];

		// see up side box can be there of not
		...
		// see right side box can be there of not
		p = current_pos + 1;
		...
		// see down side box can be there of not
		p = current_pos + this.w;
		...
		// see left side box can be there of not
		p = current_pos - 1;
	...
	_step: function (current_pos) {
		// random move from right now box
		this._target_steps = this._getTargetSteps(current_pos);
		if (this._noStep()) {
			// if no side box can be go back to last box
		...
		// random pick side box
		next_pos = this._target_steps[r];

		var is_cross = false;
		// check if alrady passed
		if (grids[next_pos])
			is_cross = true;
		...
		switch (r) {
			case 0:
				// if go up 
				// top side can pass make 1
				// down side can pass make 1 
				grids[current_pos] ^= 1;
				grids[next_pos] ^= 4;
				break;
			case 1:
				// if go right 				
				// left side can pass make 1 
				// right side can pass make 1 
				grids[current_pos] ^= 2;
				grids[next_pos] ^= 8;
				break;
			case 2:
	...
	_getNext0: function () {
		// get a none been box in the map
	...
	chkCanvasTagSupport: function () {
		// check canvas tag support or not
	...
	show: function () {
		// make statis to DOM and dispaly it
		this.clear();
...
MG.border = function (ob, v) {
	// // check  the number to know which way can pass just like my outlin description
	if (!v) {
